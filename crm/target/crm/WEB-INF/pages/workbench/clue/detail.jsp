...
<head>
	...
	$(function(){
		...
		$("#activityTbody").on("click","a",function () {
			...
			var clueId="${clue.id}";     //双引号一定要注意

			if(window.confirm("确定删除吗？")){
				//发送请求
				$.ajax({
					url:'workbench/clue/unlinkActivity.do',
					data:{
		...
		$("#activitySearchBox").keyup(function () {
			var clueId = '${requestScope.clue.id}'
			var name = this.value;

			$.ajax({
				url : 'workbench/clue/queryNotAssociatedActivity.do',
				data : {
					clueId : clueId,
					name : name
				},
				...
				success : function (data) {
					...
					$("#notAssociatedActivityList").html(htmlStr);
				}
			});
		});

		$("#linkActivityA").click(function (){
			$("activitySearchBox").val("");
		});

		$("#linkBtn").click(function (){
			var chckedIds=$("#notAssociatedActivityList input[type='checkbox']:checked");
			if (chckedIds.size() === 0) {
				alert("错误选择")
				return;
			}
			...
			$.ajax({
				url : 'workbench/clue/saveAssociatedActivity.do',
				data : ids,
				type : 'post',
				dataType : 'json',
				success : function (data) {
		...
		$("#convertBtn").click(function () {
			window.location.href = "workbench/clue/toConvert.do?clueId="+'${requestScope.clue.id}';
		});
	});
...
<body>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" id="activitySearchBox" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
					...
					<table id="activityTable" class="table table-hover" style="width: 900px; position: relative;top: 10px;">
						...
						<tbody id="notAssociatedActivityList">
							<%--<tr>
								<td><input type="checkbox"/></td>
								<td>发传单</td>
								<td>2020-10-10</td>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="linkBtn">关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" id="convertBtn" class="btn btn-default"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
		</div>
	</div>
	...
	<div style="position: relative; top: 40px; left: 40px;">
		...
		<c:forEach items="${requestScope.clueRemarkList}" var="clueRemark">
			<div class="remarkDiv" style="height: 60px;">
				...
				<div style="position: relative; top: -40px; left: 40px;" >
					<h5>哎呦！</h5>
					<font color="gray">线索</font> <font color="gray">-</font> <b>${requestScope.clue.fullname}${requestScope.clue.appellation}-${requestScope.clue.company}</b> <small style="color: gray;"> ${clueRemark.editFlag=='1'?clueRemark.editTime:clueRemark.createTime} 由${clueRemark.editFlag=='1'?clueRemark.editBy:clueRemark.createBy} ${clueRemark.editBy=='1'?'修改':'创建'}</small>
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a id="linkActivityA" href="javascript:void(0);" data-toggle="modal" data-target="#bundModal" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
